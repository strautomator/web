...
const checkAuthenticatedRoute = ({route: {matched}}) =>
    process.client
        ...
        : matched.some(({components}) => Object.values(components).some(({_Ctor}) => Object.values(_Ctor).some((c) => c.options && isAuthenticatedRoute(c))))
...
export default async (context, inject) => {
    ...
    const createAuth = (action) => (redirectUrl = context.route.fullPath) => context.redirect("/auth/" + action + "?redirect-url=" + encodeURIComponent(redirectUrl))
