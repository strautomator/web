...
WORKDIR /app
COPY . .
RUN apk update && apk upgrade && apk add --no-cache bash git openssh python3 make g++ && git config --global init.defaultBranch master && npm install --prefer-online && ./node_modules/.bin/tsc && npm run build
...
WORKDIR /app
COPY . .
RUN apk update && apk upgrade && apk add --no-cache bash git openssh python3 make g++ && git config --global init.defaultBranch master && npm install --prefer-online --production && rm -rf ./node_modules/typescript
