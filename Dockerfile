...
WORKDIR /app
COPY . .
RUN apk update && apk upgrade && apk add --no-cache bash git openssh python3 make g++ && git config --global init.defaultBranch master && npm install --production --prefer-online && rm -rf ./node_modules/typescript
